...
import { importFdx, exportFdx } from "../io/fdx";
...
const FDX_FILTER = [{ name: "Final Draft", extensions: ["fdx"] }];
...
export async function importFdxFile(store: Store, io: Io) {
  const path = await io.openDialog(FDX_FILTER);
  if (!path) return;
  const xml = await io.readTextFile(path);
  const { blocks } = importFdx(xml);
  store.setDoc(blocksToDoc(blocks));
  (store.getState().osp as OspDocument).smartTypeLists = harvest(blocks);
}

export async function exportFdxFile(store: Store, io: Io) {
  const path = await io.saveDialog("untitled.fdx", FDX_FILTER);
  if (!path) return;
  const blocks = docToBlocks(store.getState().osp.doc);
  await io.writeTextFile(path, exportFdx(blocks));
}
